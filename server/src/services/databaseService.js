...
async function upsertAgentRecords(records, dataDate) {
  ...
  if (!Array.isArray(records) || records.length === 0) {
    logger.warn('No records provided for upsert');
    return result;
  }

  if (!dataDate) {
    throw new Error('Data date is required for upsert');
  }

  // Ensure date is properly formatted
  const normalizedDate = new Date(dataDate);
  if (isNaN(normalizedDate.getTime())) {
    throw new Error(`Invalid date provided: ${dataDate}`);
  }
  ...
  try {
    for (const record of records) {
      try {
        ...
        if (!record.name) {
          ...
          logger.warn('Skipping record: missing agent name');
          continue;
        }

        // Check if record exists
        const existingRecord = await Agent.findOne({
          name: record.name,
          date: normalizedDate,
        });
        ...
        const updateResult = await Agent.findOneAndUpdate(
          ...
          },
          {
            name: record.name,
            date: normalizedDate,
            talkTime: record.talkTime || 0,
            loggedInTime: record.loggedInTime || 0,
        ...
        if (!updateResult) {
          ...
          logger.error('Upsert returned null', { agentName: record.name });
          continue;
        }

        // Count as inserted or updated based on pre-check
        if (isNewRecord) {
          result.inserted++;
        } else {
          result.updated++;
    ...
    logger.info('Agent records upsert completed', {
